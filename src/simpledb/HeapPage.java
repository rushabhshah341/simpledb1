...
public class HeapPage implements Page {
	...
	public Tuple getTuple(int entryID) {
		...
		Tuple insertedRecord = createTuple(in);
		int entryCount = entryCount();
		if(entryID < entryCount) { 			//checking entryid < 20 or not
			insertedRecord.setRecordId(new RecordId(pid, entryID));
	...
	ArrayList<Tuple> al = new ArrayList<>(); 	//New instance of arraylist in which tuples will be stored.
		for(int i=0;i<entryCount();i++) { 		//running till end of the entrycounts
			al.add(getTuple(i));				//Adding tuple to our new array list which will be iterated later. 
		}
		return al.iterator();   				//Returning Iterator.
	}
	
	/*private class dataIterator implements Iterator<Tuple>
	{
		 int pointer;       // index of next element to return
	     int lastElemnentRet = -1; // index of last element returned; -1 if no such
	     int expectedCount = entryCount();
	     private int size;
	        ...
	        public Tuple next() {
	            ...
	            pointer = i + 1;
	            return (Tuple) elementData[lastElemnentRet = i];
	        }
