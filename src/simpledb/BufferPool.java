...
public class BufferPool {
	...
	public Page getPage(TransactionId tid, PageId pid, Permissions perm)
		...
		Page page;
		if(id2page.get(pid)!=null) // Checking weather page is in Bufferpool or not
		{
			page = id2page.get(pid);
			return page;
		}else {
			if(id2page.size()<numPages){  //Checking weather bufferpool is full or not and id2page's size is less than maximum number of pages
					page =  Database.getCatalog().getDbFile(pid.getTableId()).readPage(pid);
					id2page.put(pid, page);
					return page;
				}else{
					throw new DbException("This can not be done.");
			}	
		}
	}
